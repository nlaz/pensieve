...
import { CREATE_DECK, FETCH_DECKS, FETCH_DECK, EDIT_DECK, DELETE_DECK, UPDATE_MESSAGE, DECK_ERROR } from './types';
...
export function deleteDeck(itemId) {
  ...
  return function (dispatch) {
    axios.delete(`/api/decks/${itemId}`, config)
      .then(() => {
      ...
      .catch((error) => {
        dispatch({
          ...
          payload: {
            error: error.response.data.error,
            message: 'There was a problem removing your deck. Doublecheck your network connection and try again.',
          }
        });
      });
  };
